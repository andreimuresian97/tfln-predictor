...
st.markdown("""
Instant inference for **VPI, nm, and Z0**.  
...
def predict_sequentially(geometry, L_cm):
    ...
    # REMOVED S21 from targets
    targets = {'VPI': 'VPI (duty cycle)', 'nm': 'nm', 'Z0': 'Z0'}
    ...
    for safe_name, scaler_key in targets.items():
        ...
        idx += 1
        progress.progress(idx / total_targets) # Adjusted progress bar math
...
if st.button("Predict Performance", type="primary"):
    ...
    if results:
        # Adjusted layout from 4 columns to 3
        col1, col2, col3 = st.columns(3)
        col1.metric("VPI", f"{results['VPI']['value']:.2f} V")
        col2.metric("Impedance (Z0)", f"{results['Z0']['value']:.1f} Ω")
        col3.metric("Index (nm)", f"{results['nm']['value']:.3f}")
