...
st.title("🎯 GP Surrogate-Driven TFLN Optimizer")
st.markdown("""
**Find the absolute physical limit of your design.**
This tool uses a **Hybrid Bayesian Optimization** engine (Sobol Sequence Global Search + SLSQP Gradient Polishing) 
to find the optimal TFLN cross-section for your specific performance constraints.
""")

# ==========================================
# 2. SIDEBAR: USER INPUTS
# ==========================================
st.sidebar.header("1. Global Device Parameters")
...
st.sidebar.markdown("Define the allowable search range (µm).")
...
const_vpi = st.sidebar.number_input("Max Vpi (V) [Adjusted for Length]", value=2.0, step=0.1)
...
beta = st.sidebar.slider("Optimization Aggressiveness (Beta)", 0.0, 2.0, 0.5, help="0.0 = Safe/Exploitation, 2.0 = Curious/Exploration")
...
def predict_batch_memory_safe(X_norm, scalers, L_cm):
    """
    Loads models sequentially to save RAM.
    Returns a dictionary of predictions with length-scaled VPI.
    """
    preds = {}
    model_names = ['VPI', 'nm', 'Z0', 'S21']
    
    prog_bar = st.progress(0, text="Evaluating 200,000 Geometries...")
    ...
    for i, name in enumerate(model_names):
        ...
        # Post-Processing: VPI Log extraction and Length scaling
        if name == 'VPI':
            mean_phys = 10 ** mean_phys
            std_phys = mean_phys * np.log(10) * std_phys
            mean_phys = mean_phys / L_cm
            std_phys = std_phys / L_cm
...
def run_optimization(length_cm):
    ...
    with st.spinner(f"Generating {N_CANDIDATES} Sobol Sequence Geometries..."):
    ...
    st.caption(f"Valid Geometries Sent to Surrogate: {len(X_valid)} / {N_CANDIDATES}")

    # 3. Batch Predict
    scaler_X = scalers['X']['input']
    X_norm = scaler_X.transform(X_valid)
    
    preds = predict_batch_memory_safe(X_norm, scalers, length_cm)
    if not preds: return
    ...
    if len(valid_indices) == 0:
        st.warning(f"No designs met the required physics constraints. Try relaxing them.")
        return
    ...
    # 6. Polishing (SLSQP)
    perform_polishing(winner_x, scalers, length_cm)

def perform_polishing(x0, scalers, length_cm):
    st.info("🛠 Initiating Gradient Optimization (SLSQP) to pinpoint the mathematical global maximum...")
...
if 'result' in st.session_state:
    ...
    c3.metric("VPI", f"{preds['VPI']:.2f} V", delta_color="inverse")
