...
def user_input_features():
    ...
    # Return dictionary for easier access
    return {
...
# Prepare list for the Predictor (Order must match training!)
...
# --- PLOTTING LOGIC (Adapted from your script) ---
def generate_plots(p):
    """
    Generates Matplotlib figures for Top-Down and Cross-Section views.
    """
    # Unpack parameters
    ...
    def draw_arrow(ax, x1, y1, x2, y2, text, loc, a_offset, t_offset):
        ...
        ax.text(mid_x + off_x, mid_y + off_y, text, ha=ha, va=va, fontsize=FONT_SIZE, color=LINE_COLOR, zorder=21)
    ...
    # Electrodes
    ax2.add_patch(patches.Rectangle((ws_right + GAP, base_y), WG_FIXED, MTX, fc=ELECTRODE_COLOR, ec=LINE_COLOR)) # Right
    ax2.add_patch(patches.Rectangle((ws_left, base_y), WS, MTX, fc=ELECTRODE_COLOR, ec=LINE_COLOR)) # Center (Signal)
    ax2.add_patch(patches.Rectangle((ws_left - GAP - WG_FIXED, base_y), WG_FIXED, MTX, fc=ELECTRODE_COLOR, ec=LINE_COLOR)) # Left
    ...
    ax2.set_xlim(ws_left - GAP - 10, ws_right + GAP + 10) # Zoomed view
...
if st.button("Predict Performance", type="primary"):
    try:
        ...
        st.info("Ensure the 'gp_surrogate_results_199_8var_fixed' folder is in the same directory.")
